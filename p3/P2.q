...
-- view
--=============================================================================
DROP VIEW IF EXISTS CountByBirthMonthDay;
CREATE VIEW CountByBirthMonthDay AS
SELECT 
CONCAT(m.birthMonth, '/', m.birthDay) AS birthMonDay, 
COUNT(*) AS cnt, 
DENSE_RANK () OVER (ORDER BY COUNT(*) DESC) AS rnk
FROM master AS m
WHERE m.birthMonth IS NOT NULL AND m.birthDay IS NOT NULL
GROUP BY CONCAT(m.birthMonth, '/', m.birthDay);

--=============================================================================
-- select
--=============================================================================
SELECT birthMonDay
FROM CountByBirthMonthDay
WHERE rnk <= 3;
