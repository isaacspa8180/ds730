...
-- view
--=============================================================================
DROP VIEW IF EXISTS PlayerErrorCount;
CREATE VIEW PlayerErrorCount AS
SELECT f.playerID, SUM(f.E) AS tot_e, DENSE_RANK() OVER (ORDER BY SUM(f.E) DESC) AS rnk
FROM fielding AS f
GROUP by f.playerID;
